...
class Renderer(xsl2.Renderer):
    ...
    fork = TagProp('fork')
    function = TagProp('function', {
        'name', 'as', 'visibility', 'streamability', 'override-extension-function', 'new-each-time',
        'cache'
    })
    ...
    merge_source = TagProp('merge-source', {
        'name', 'for-each-item', 'for-each-source', 'select', 'streamable', 'use-accumulators',
        'sort-before-merge', 'validation', 'type'
    })
    message = TagProp('message', {'select', 'terminate', 'error-code'})
    mode = TagProp('mode', {
        'name', 'streamable', 'use-accumulators', 'on-no-match', 'on-multiple-match',
    ...
    output = TagProp('output', {
        'name', 'method', 'allow-duplicate-names', 'build-tree', 'byte-order-mark',
        'cdata-section-elements', 'doctype-public', 'doctype-system', 'encoding',
        'escape-uri-attributes', 'html-version', 'include-content-type', 'indent',
        'item-separator', 'json-node-output-method', 'media-type', 'normalization-form',
        'omit-xml-declaration', 'parameter-document', 'standalone', 'suppress-indentation',
        'undeclare-prefixes', 'use-character-maps', 'version'
    })
    override = TagProp('override')
    package = TagProp('package', {
        'id', 'name', 'package-version', 'version', 'input-type-annotations', 'declared-modes',
        'default-mode', 'default-validation', 'default-collation', 'extension-element-prefixes',
        'exclude-result-prefixes', 'expand-text', 'use-when', 'xpath-default-namespace'
    })
    param = TagProp('param', {'name', 'select', 'as', 'required', 'tunnel', 'static'})
    result_document = TagProp('result-document', {
        'format', 'href', 'validation', 'type', 'method', 'allow-duplicate-names', 'build-tree',
        'byte-order-mark', 'cdata-section-elements', 'doctype-public', 'doctype-system', 'encoding',
        'escape-uri-attributes', 'html-version', 'include-content-type', 'indent', 'item-separator',
        'json-node-output-method', 'media-type', 'normalization-form', 'omit-xml-declaration',
        'parameter-document', 'standalone', 'suppress-indentation', 'undeclare-prefixes',
        'use-character-maps', 'output-version'
    })
    source_document = TagProp('source-document', {
        'href', 'streamable', 'use-accumulators', 'validation', 'type'
    })
    stylesheet = TagProp('stylesheet', {
        'id', 'version', 'default-mode', 'default-validation', 'input-type-annotations',
        'default-collation', 'extension-element-prefixes', 'exclude-result-prefixes',
        'expand-text', 'use-when', 'xpath-default-namespace'
    })
